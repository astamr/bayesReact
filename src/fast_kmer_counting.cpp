...
// only uppercase A/C/G/T in (motifs and) seqs; maps A,C,G,T (nucleotide) -> 0,1,2,3 (code).
...
extern "C" SEXP fast_kmers_counting(SEXP seqs, SEXP k_len) {
  ...
  // allocate and protect an n_seqs × n_kmers R matrix, initialize all counts to zero, and obtain direct C++ access to its numeric values.
  ...
  // positions after an accepted match. `release` records when it can be counted again.
  ...
  // scan each sequence independently. Reset the sliding k-mer code window, sequence
  ...
  for (int seq_index = 0; seq_index < n_seqs; ++seq_index) {
    ...
    for (const unsigned char* p =
         ...
         *p != '\0'; ++p) {
      // append the current nt to the sliding k-mer code window and discard the nt that has moved outside the current k-mer window.
      ...
      release[kmer_unblock_index] = -1; // clear the current release record before next seq.

      // count a given k-mer only if it was not counted at an overlapping start position.
      // then block it until it becomes eligible again.
      if (!blocked[kmer_code]) {
        //motif_counts_i[seq_index + n_seqs * kmer_code] += 1.0; // updated to avoid overflow
    ...
    // unblock k-mers still blocked at the end of a sequence before scanning the next sequence.
