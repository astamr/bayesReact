...
#'
#' @param motifs a vector of motifs (e.g., c("ACGTAGT", "GTACAAG")); an integer specifying the k-mers to be used (e.g., 7 for all 7-mers); or a list of 'aligned motif sites'/PCM/PPM/PWM matrices (e.g., list(mot1 = matrix(nrow = 4, dimnames = list(c("A", "C", "G", "T"), NULL)))). A motif is specified as a regular expression on the alphabet {A,C,G,T}.
...
#' @param binom_approx boolean specifying whether to use a binomial approximation to calculate motif probabilities or the default n-step TPM assuming an underlying Markov chain.
#' The binomial method assumes a random and independent distribution of nucleotides in the sequence of interest.
#' The approach is most appropriate when considering a large set of longer k-mers (> 7-mers). If binom_approx = T, 'motifs' has to be specified as a k-mer (e.g., motifs = 8 for all 8-mers) or as a list of degenerative motifs.
...
#' @param pwm_motif_type character specifying the motif representation. Should be either "motif_sites", "pcm" (nt counts), "ppm" (default; contains nt freq/prob in motif), or "pwm" (log-odds ratios; see pwm_motif_prob() for more detail).
#'
...
motif_prob <- function(motifs, seqs, seqlist, paths = T, binom_approx = F, cores = parallel::detectCores(),
                       out_path = NULL, include_counts = T, markov_order = 0, pwm_motif_type = "ppm") {
  ...
  if(is.list(motifs_int)){ # "global" currently unreachable!! (dp is better approx.)
    ...
    if (is.character(out_path)) { # return immediately
      if(substring(out_path, nchar(out_path)) != "/"){out_path <- paste0(out_path, "/")}
      saveRDS(pwm_out$motif_counts, file = paste0(out_path, "seqXmot_counts.rds"))
      saveRDS(pwm_out$motif_probs, file = paste0(out_path, "seqXmot_probs.rds"))
      ...
      return(list(motif_probs = pwm_out$motif_probs, motif_counts = pwm_out$motif_counts))
    }
  }
  ...
  if (binom_approx == F) {
    ...
    binom_ssp <- if (markov_order == 0) { # 0th-order: SSP from nt freqs
      function(m){
        mot <- motif_idx[[m]]
        SSP_m <- numeric(S)
    ...
    } else if (markov_order == 1) { # 1st-order: SSP from dinucleotide context via conditional probs
      function(m){
        mot <- motif_idx[[m]]
        SSP_m <- numeric(S)
